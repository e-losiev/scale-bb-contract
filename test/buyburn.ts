...
describe("SCALE BUY BURN", function () {
    describe("Deployment && Errors", function () {
        ...
        it("Should revert with no balance", async function () {
            ...
            await expect(buyburn.connect(user).buyAndBurn(0, 0, 0)).to.be.revertedWithCustomError(buyburn, "NoAllocation");
        });
        it("Should revert non-whitelisted", async function () {
            const { buyburn, user2 } = await loadFixture(buyBurnFundedE280Fixture);
            await expect(buyburn.connect(user2).buyAndBurn(0, 0, 0)).to.be.revertedWithCustomError(buyburn, "Prohibited");
        });
    });
    describe("BuyBurn", function () {
        it("Should perform a buy burn using e280 only", async function () {
            const { buyburn, user, e280, scale, capPerSwapE280, buyBurnInterval } = await loadFixture(buyBurnFundedE280Fixture);
            ...
            const scaleAmountOut = await getQuoteV2(e280.target, scale.target, allocationAfterIncentive);
            const scaleAfterTax = scaleAmountOut - calculateBPS(scaleAmountOut, 700);
            const minScaleAmount = applySlippage(scaleAfterTax);
            const deadline = await getDeadline();
            await expect(buyburn.connect(user).buyAndBurn(minScaleAmount, 0, deadline)).to.changeTokenBalances(
                e280,
                [buyburn, user],
            ...
            await expect(buyburn.connect(user).buyAndBurn(0, 0, 0)).to.be.revertedWithCustomError(buyburn, "Cooldown");
        });
        it("Should perform a buy burn using dragonx only - dragonx check", async function () {
            const { buyburn, user, e280, dragonx, scale, capPerSwapE280, capPerSwapDragonX, buyBurnInterval } = await loadFixture(
                buyBurnFundedDragonXFixture
            );
            const [additionalSwap, nextE280Swap, nextDragonXSwap, nextBuyBurn] = await buyburn.getBuyBurnParams();
